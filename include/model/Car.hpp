...
struct EnergyUpdate {
  ...
  EnergyUpdate(double power, double energy)
      : power(power), energy(energy), force(0.0) {}
  EnergyUpdate(double power, double energy, double force)
      : power(power), energy(energy), force(force) {}
...
struct CarUpdate {
  ...
  explicit CarUpdate(EnergyUpdate aero = EnergyUpdate(),
                     ...
                     double motor_power = 0.0, double motor_energy = 0.0,
                     double bearing = 0.0, double electric = 0.0,
                     double delta_energy = 0.0, double delta_distance = 0.0,
...
/** Common geometry parameters that must be calculated when computing energy
 * losses */
struct Geometry {
  double bearing;
  double sin_theta;
  ...
  explicit Geometry(double bearing = 0.0, double sin_theta = 0.0,
                    double cos_theta = 0.0, double distance = 0.0,
                    util::type::SolarAngle az_el = util::type::SolarAngle())
      : bearing(bearing),
        sin_theta(sin_theta),
        cos_theta(cos_theta),
...
struct CarParams {
  double mass;                      // kg
  double cda;                       // Unitless
  double motor_efficiency;          // Unitless
  double regen_efficiency;          // Unitless (unused)
  double battery_efficiency;        // Unitless
  double passive_electric_loss;     // Watts
  double air_density;               // kg / m^3
  double array_area;                // m^2
  double max_soc;                   // kWh
  double tire_pressure;             // bar
  double array_efficiency;          // Unitless
  double max_braking_force;         // Newtons
  double max_motor_power;           // Watts
  EffLut yint_rolling_resistance;   // Unitless
  EffLut slope_rolling_resistance;  // s / m
  BasicLut power_factors;           // m^2

  CarParams(double mass, double cda, double motor_efficiency,
            double regen_efficiency, double battery_efficiency,
            double passive_electric_loss, double air_density, double array_area,
            double max_soc, double tire_pressure, double array_efficiency,
            double max_braking_force, double max_motor_power,
            EffLut yint_rolling_resistance, EffLut slope_rolling_resistance,
      ...
      : mass(mass),
        ...
        passive_electric_loss(passive_electric_loss),
        air_density(air_density),
        array_area(array_area),
        max_soc(max_soc),
        tire_pressure(tire_pressure),
        array_efficiency(array_efficiency),
        max_braking_force(max_braking_force),
...
inline CarParams get_car_parameters(ConfigParser* parser) {
  RUNTIME_EXCEPTION(parser != nullptr,
                    "Parser is null when retrieving Car parameters");
  return CarParams(
      parser->get_car_mass(), parser->get_cda(), parser->get_motor_efficiency(),
      parser->get_regen_efficiency(), parser->get_battery_efficiency(),
      parser->get_passive_electric_loss(), parser->get_air_density(),
      parser->get_array_area(), parser->get_max_soc(),
      parser->get_tire_pressure(), parser->get_array_efficiency(),
      parser->get_max_braking_force(), parser->get_max_motor_power(),
      EffLut(parser->get_roll_res_yint_path()),
      EffLut(parser->get_roll_res_slope_path()),
...
class Car {
 private:
  ...
  static constexpr int num_data_points_per_second = 10;

 public:
  explicit Car(CarParams params);

  [[nodiscard]] double get_max_soc() const { return params.max_soc; }
   ...
   */
  [[nodiscard]] EnergyUpdate compute_aero_loss(double speed, double car_bearing,
                                               util::type::Wind wind,
   ...
   */
  [[nodiscard]] EnergyUpdate compute_rolling_loss(double speed,
                                                  double delta_time,
   ...
   */
  [[nodiscard]] EnergyUpdate compute_gravitational_loss(double speed,
                                                        double delta_time,
   ...
   * @param delta_time: Time interval in seconds
   */
  [[nodiscard]] double compute_electric_loss(double delta_time) const;
   ...
   */
  [[nodiscard]] EnergyUpdate compute_array_gain(double delta_time,
   ...
   */
  [[nodiscard]] static Geometry calculate_geometry(
      util::type::Coord coord_one, util::type::Coord coord_two,
      const util::type::Time& time, double delta_distance = -1.0);

  /** @brief Calculate motor energy loss */
  [[nodiscard]] MotorEnergyLoss calculate_motor_loss(
      double init_speed, double acceleration, double delta_time,
      const Geometry& g, util::type::Wind wind) const;

  /** @brief Compute energy change when moving between two points in a straight
   ...
   *
   * @return CarUpdate: State updates to the car after moving from coord_one to
   * coord_two
   */
  [[nodiscard]] CarUpdate compute_constant_travel_update(
      util::type::Coord coord_one, util::type::Coord coord_two, double speed,
      const util::type::Time& time, util::type::Wind wind,
      util::type::Irradiance irr, double distance = -1.0) const;

  /** @brief Compute enrgy change when moving between two points in a straight
   * line with some acceleration
   *
   * @param coord_one: Starting coordinate
   * @param coord_two: Ending coordinate
   ...
   * @param time: Current time at coord_one
   * @param wind, irr: Weather forecast taken at coord_one
   * @param delta_distance: The distance between the coordinates. Can be passed
   * in if using pre-computed distances
   */
  [[nodiscard]] CarUpdate compute_acceleration_travel_update(
      util::type::Coord coord_one, util::type::Coord coord_two,
      double init_speed, double acceleration, const util::type::Time& time,
      util::type::Wind wind, util::type::Irradiance irr,
   ...
   */
  [[nodiscard]] CarUpdate compute_dual_travel_update(
      util::type::Coord coord_one, util::type::Coord coord_two,
      double init_speed, double acceleration, const util::type::Time& time,
      util::type::Wind wind, util::type::Irradiance irr,
      double acceleration_distance, double constant_distance) const;
   ...
   * @param irr: Irradiance of the sun (assumed to be constant over the entire
   * stop)
   */
  [[nodiscard]] double compute_static_energy(util::type::Irradiance irr,
                                             util::type::SolarAngle az_el,
