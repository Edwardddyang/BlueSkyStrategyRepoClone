...
struct ASCSimulatorParams : SimulatorParams {
  int checkpoint_hold_time;  // Stop time at a control stop in seconds
  double
      sim_start_soc;  // Starting soc for the simulation. Retrieved from config
  ...
  ASCSimulatorParams(ForecastMatrix wind_speed_lut, ForecastMatrix wind_dir_lut,
      ...
      : SimulatorParams(std::move(wind_speed_lut), std::move(wind_dir_lut),
        ...
        checkpoint_hold_time(checkpoint_hold_time),
...
inline ASCSimulatorParams get_asc_simulator_params(ConfigParser* parser) {
  RUNTIME_EXCEPTION(
      parser != nullptr,
      "Config parser is null when loading ASC simulation parameters");
