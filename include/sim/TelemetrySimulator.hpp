...
struct TelemetrySimulatorParams : SimulatorParams {
  ...
  TelemetrySimulatorParams(ForecastMatrix wind_speed_lut,
                           ForecastMatrix wind_dir_lut, ForecastMatrix dni_lut,
                           ForecastMatrix dhi_lut, double sim_start_soc)
...
inline TelemetrySimulatorParams get_telem_simulator_params(
    ConfigParser* parser) {
  RUNTIME_EXCEPTION(
      parser != nullptr,
      "Config parser is null when loading telemetry simulation parameters");
  ...
  TelemetrySimulatorParams params{
      ...
      ForecastMatrix(parser->get_dhi_path()), parser->get_current_soc()};

  return params;
}

/** @brief Simulator to simulate the path taken by telemetry data through a csv
 * loaded as |Latitude|Longitude|Altitude|Time|Speed
 */
class TelemetrySimulator {
 ...
 public:
  ...
  void run_sim(const TelemRoute& route, Luts::DataSet* results_lut);
};
