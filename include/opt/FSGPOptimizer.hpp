/** Optimizer that searches for the best acceleration profile using a genetic
 * algorithm */
#pragma once
...
class FSGPOptimizer
    : public Optimizer<FSGPOptimizer, FSGPRoute, FSGPRacePlan, FSGPSimulator> {
 private:
  ...
  // These parameters are required for mutating RacePlans and crossing over
  // parents
  const double max_motor_power;
  const double car_mass;  // kg
  const double
      acceleration_power_budget;  // Units of Watts, must be < max_motor_power
  ...
  ///////////////////////// Crossover ////////////////////////
  ////////////////////////////////////////////////////////////
  std::unordered_set<std::string> CROSSOVER_STRATEGIES = {"LoopCross"};
  ...
  RacePlan loop_cross(RacePlan* parent_a, RacePlan* parent_b,
                      RacePlanCreator::Gen* gen);
  ...
  /** @brief Mutate a race plan according to some strategy chosen from
   * configuration */
  ...
  /** @brief Add either +1 or -1 m/s to the end of an acceleration/deceleration
   * segment */
  void acceleration_noise(FSGPRacePlan* plan, RacePlanCreator::Gen* rng);
  /** @brief Mutate a loop of the plan using the method described above
   * @note new_raw_plan is modified in place
   */
  void acceleration_noise_loop(FSGPRacePlan::PlanData* new_raw_plan,
   ...
   * @note new_raw_plan is modified in place
   */
  void constant_noise_loop(FSGPRacePlan::PlanData* new_raw_plan,
                           size_t loop_idx, RacePlanCreator::Gen* rng);

  /** @brief Mutate each loop using one of the techniques above */
  void mix_mutation(RacePlan* plan, RacePlanCreator::Gen* rng);

  /** @brief Attempt to legalize a loop starting from some segment. Modification
   ...
   * Loop Indices: [0,5],[5,14],[16,26] -> Discontinuity going from segment 1 to
   * segment 2 Loop Speeds: [14,14],[14,15],[13,13] -> Discontinuity going from
   * segment 1 to segment 2 Discontinuities are rectified by attemping to
   * propagate "truth" from replacement_idx
   *
   * @return True if legalization was successful, false if unsuccessful
   */
  bool legalize_loop(FSGPRacePlan::PlanData* plan, size_t loop_idx,
                     size_t seg_idx, FileLogger* logger = nullptr);
