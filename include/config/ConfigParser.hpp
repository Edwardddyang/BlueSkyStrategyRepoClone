...
/* Define all config parameters via. X-Macro
   PARAM(
      <parameter name as it appears exactly in the .yaml configuration file>,
        ...
        If the parameter you're introducing is a pointer of some type, please
  use unique_ptr as the <data type> to avoid memory leaks. If you're introducing
  a new datatype, you'll also have to make relevant changes in ConfigParam.hpp.
  Note that for all variables of std::filesystem::path type, $STRAT_ROOT will be
...
#define CONFIG_PARAMETERS                                                      \
  ...
  PARAM(overnight_charging_location, util::type::Coord,                        \
        util::type::Coord(37.000823, -86.367676, 0.157135))                    \
  ...
  PARAM(day_one_start_time, util::type::Time,                                  \
        util::type::Time("2024-08-16T10:00:00+00:00"))                         \
  PARAM(day_one_end_time, util::type::Time,                                    \
        util::type::Time("2024-08-16T18:00:00+00:00"))                         \
  PARAM(day_start_time, util::type::Time,                                      \
        util::type::Time("2024-08-17T09:00:00+00:00"))                         \
  PARAM(day_end_time, util::type::Time,                                        \
        util::type::Time("2024-08-17T17:00:00+00:00"))                         \
  PARAM(race_end_time, util::type::Time,                                       \
        util::type::Time("2024-08-26T17:00:00+00:00"))                         \
  PARAM(impounding_release_time, util::type::Time,                             \
        util::type::Time("2024-08-26T07:00:00+00:00"))                         \
  PARAM(impounding_start_time, util::type::Time,                               \
        util::type::Time("2024-08-26T07:20:00+00:00"))                         \
  PARAM(current_date_time, util::type::Time,                                   \
        util::type::Time("2024-08-26T07:20:00+00:00"))                         \
  PARAM(checkpoint_hold_time, int, 30)

/** Parse the config .yaml file
 * the ::load_config(file) method should be called immediately after
 * construction
 */
class ConfigParser {
 private:
...
#define PARAM(name, type, default_value) ConfigParam<type> name;  // NOLINT
...
#undef PARAM
 ...
 public:
  ...
  explicit ConfigParser(const std::filesystem::path& file_path);
  ConfigParser() = delete;

/* Getters */
#define PARAM(name, type, default_value)      \
  inline ParamReturnType<type> get_##name() { \
...
#undef PARAM
  ...
  std::string get_strat_root() { return STRAT_ROOT; }
