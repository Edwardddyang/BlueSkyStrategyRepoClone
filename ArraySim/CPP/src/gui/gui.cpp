...
void GUI::render_step_two_layout() {    
    ...
    if (ImGui::Button("Generate Irradiance CSV")) {
        ...
        generate_irradiance_csv();
        
    }
}

void GUI::generate_irradiance_csv() {
...
glm::vec3 findOrthogonalVector(const glm::vec3& normal) {
...
std::vector<glm::vec2> convexHull2D(const std::vector<glm::vec2>& points) {
...
void GUI::compute_canopy_shadow(std::shared_ptr<SunPlane>& sun_plane) {
    ...
    std::vector<std::vector<Vertex>> cell_vertices = car_model->get_array_cell_vertices();
