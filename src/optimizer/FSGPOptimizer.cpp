...
void thread_create_plan(std::shared_ptr<RacePlanCreator> generator,
                        FSGPRacePlan* space, ThreadManager* thread_manager) {
...
FSGPRacePlan V2Optimizer::optimize() {
  ...
  for (size_t i = 0; i < num_generations + 1; i++) {
    ...
    mutation_logger("//////////////////////////////////////////");
    mutation_logger("// Mutating population in generation " +
                    std::to_string(i) + " //");
    mutation_logger("//////////////////////////////////////////\n");
  ...
  if (save_csv) {
    best_race_plan_result.write_logs(
        (results_folder / "Acceleration.csv").string());
  }

  if (best_race_plan.is_viable()) {
    best_race_plan.print_plan();
    ResultsLut best_race_result;
    std::cout << "Best Race Plan Average Speed: "
              << mps2kph(best_race_plan.get_average_speed()) << "kph"
              << std::endl;
    std::cout << "Best Race Plan Number of Loops: "
              << best_race_plan.get_num_loops() << std::endl;
  } else {
    std::cout << "No plan is viable" << std::endl;
    std::cout << "Inviability reason: "
              << best_race_plan.get_inviability_reason() << std::endl;
  }
...
void V2Optimizer::mutate_population() {
  std::uniform_int_distribution<size_t> indices_dist(crossover_num,
                                                     population_size - 1);
...
void V2Optimizer::crossover_population() {
  ...
  std::uniform_int_distribution<size_t> parent_indices_dist(0,
                                                            crossover_num - 1);
  ...
  RUNTIME_EXCEPTION(
      crossover_num != 0 && static_cast<int>(crossover_num) % 2 == 0,
      "Number of the population to crossover is not divisible by 2 or not "
      "greater than 0");
...
FSGPRacePlan V2Optimizer::crossover_parents(FSGPRacePlan parent_a,
                                            FSGPRacePlan parent_b) {
  RUNTIME_EXCEPTION(!parent_a.is_empty() && !parent_b.is_empty(),
                    "Parents of crossover cannot be empty");
  const std::string crossover_type =
      Config::get_instance().get_crossover_strategy();
  RUNTIME_EXCEPTION(
      CROSSOVER_STRATEGIES.find(crossover_type) != CROSSOVER_STRATEGIES.end(),
...
// FSGPRacePlan V2Optimizer::splice_loop(FSGPRacePlan* parent_a, FSGPRacePlan*
// parent_b, FSGPRacePlanCreator::Gen* rng) {
...
//   const FSGPRacePlan::PlanData a_orig_segments =
//   parent_a->get_orig_segments(); const FSGPRacePlan::PlanData b_orig_segments
//   = parent_b->get_orig_segments();
//   RUNTIME_EXCEPTION(parent_a->get_num_loops() == parent_b->get_num_loops(),
//                     "Parent a and b must have the same number of loops to use
//                     LoopCross crossover strategy");
//   RUNTIME_EXCEPTION(a_orig_segments.size() == b_orig_segments.size(),
//                     "Both parents must have the same number of original
//                     segments");
//   std::uniform_int_distribution<int> parent_dist(0, 1);
//   FSGPRacePlan::PlanData new_raw_plan;
//   FSGPRacePlanCreator::PlanAttributes att;
//   const std::unordered_map<size_t, size_t> corner_end_to_corner_idx =
//   this->route->get_corner_end_map();

//   const size_t num_blocks = a_orig_segments.size();
//   for (size_t block_idx=0; block_idx < num_blocks; block_idx++) {
//     RUNTIME_EXCEPTION(a_orig_segments[block_idx].size() ==
//     b_orig_segments[block_idx].size(),
...
//           corner_end_to_corner_idx.find(end_idx) !=
//           corner_end_to_corner_idx.end()) {
...
//     std::uniform_int_distribution<int> idx_dist(0, overlapping_corners.size()
//     - 1); const size_t splice_idx = idx_dist(rng->idx_rng); if (block_idx ==
//     0) {
//       // Is empty
...
//                     a_orig_segments[block_idx].begin(),
//                     a_orig_segments[block_idx].begin() + splice_idx + 1);
...
//     // ----A------|-----B------: Have to modify the connection segment
//     between A and B by matching the
...
//     new_raw_plan[block_idx][splice_idx+1].acceleration_value =
//     calc_acceleration(
...
//     // We have to insert the next loop from parent B to interface with
//     legalize_loop correctly if (!legalize_loop(&new_raw_plan, ))
//   }

// }

FSGPRacePlan V2Optimizer::loop_cross(FSGPRacePlan* parent_a,
                                     FSGPRacePlan* parent_b,
                                     RacePlanCreator::Gen* rng) {
  RUNTIME_EXCEPTION(parent_a != nullptr && !parent_a->is_empty(),
                    "Parent a cannot be null and must not be empty");
  RUNTIME_EXCEPTION(parent_b != nullptr && !parent_b->is_empty(),
                    "Parent b cannot be null and must not be empty");
  ...
  RUNTIME_EXCEPTION(parent_a->get_num_loops() == parent_b->get_num_loops(),
                    ...
                    "LoopCross crossover strategy");
  RUNTIME_EXCEPTION(
      a_orig_segments.size() == b_orig_segments.size(),
  ...
  // The speed at the last real corner from the previous loop (connection
  // segments start here)
  ...
  for (size_t block_idx = 0; block_idx < num_blocks; block_idx++) {
    ...
    if (parent_idx == 0) {  // Take loop from parent A
      crossover_logger("Picked parent A for block " +
      ...
      crossover_logger("Picked parent B for block " +
                       std::to_string(block_idx));
    ...
    if (block_idx != 0) {
      ...
      new_raw_plan[block_idx][0].acceleration_value =
          calc_acceleration(new_raw_plan[block_idx][0].start_speed,
                            new_raw_plan[block_idx][0].end_speed,
                            new_raw_plan[block_idx][0].distance);
      if (!legalize_loop(&new_raw_plan, block_idx, 0, &crossover_logger)) {
        crossover_logger(
            "Legalization of loop failed, adding loop of the previous parent");
        new_raw_plan[block_idx] = prev_loop;
      }
    }

    att.raw_segments.push_back(new_raw_plan[block_idx]);
    generator->create_loop_block(
        &new_raw_plan[block_idx], this->num_loops_in_block,
        block_idx == num_blocks - 1, block_idx == 0, &att, nullptr);

    // Assign the speed of the connection corner
    if (block_idx != num_blocks - 1) {
      const FSGPRacePlan::LoopData plan = parent_idx == 0
                                              ? a_orig_segments[block_idx + 1]
  ...
  FSGPRacePlan new_plan(att.all_segments, att.raw_segments,
...
void V2Optimizer::init_params() {
  ...
  RUNTIME_EXCEPTION(population_size > 0,
                    "Initial population size must be greater than 0");
  RUNTIME_EXCEPTION(num_generations > 0,
                    "Number of generations must be greater than 0");
  RUNTIME_EXCEPTION(
      crossover_percentage + survival_percentage + mutation_percentage == 100.0,
      "Survival, "
  ...
  if (crossover_percentage != 0.0) {
    RUNTIME_EXCEPTION(raw_crossover_num >= 2,
                      "Parents percent must select at least two parents to "
                      "breed each generation. "
                      "(crossover_percentage / 100.0) * population_size is {} "
                      "which is less than 2",
                      raw_crossover_num);
  }
  if (mutation_percentage != 0.0) {
    RUNTIME_EXCEPTION(raw_mutation_num >= 1,
                      "Mutation percentage must select at least one "
                      "parent to mutate each generation. (mutation_percentage "
                      "/ 100.0) * population_size is {} which is less than 1",
                      raw_mutation_num);
  }
  ...
  spdlog::info("Survival Percentage: {}, Number of Population {}",
               survival_percentage, survival_num);
  spdlog::info("Mutation Percentage: {}, Number of Population: {}",
               mutation_percentage, mutation_num);
  spdlog::info("Crossover Percentage: {}, Number of Population: {}",
               crossover_percentage, crossover_num);

  if (log_optimization && population_size > 1) {
    spdlog::warn(
        "Population size is greater than 1 with optimization logging set to "
        "true. "
  ...
  rng_collection = RacePlanCreator::Gen(speed_seed, loop_seed, aggressive_seed,
                                        idx_seed, acceleration_seed, skip_seed);
}

void V2Optimizer::create_initial_population() {
  RUNTIME_EXCEPTION(population_size > 0,
                    "Initial population size must be greater than 1");
  RUNTIME_EXCEPTION(population.size() == population_size,
                    "Population vector not resized");
  RUNTIME_EXCEPTION(result_luts.size() == population_size,
                    "Results lut vector not resized");
  ...
  if (population_size > 1) {
    for (int i = 0; i < population_size; i++) {
      threads[i] = std::thread(thread_create_plan, generator, &population[i],
                               &thread_manager);
    }
    for (int i = 0; i < population_size; i++) {
      threads[i].join();
...
void V2Optimizer::print_population_status(int generation) {
  ...
  std::cout << "Population Average Speed: " << mps2kph(population_average_speed)
            << "kph" << std::endl;
  std::cout << "Number of Viable Plans: " << num_viable_plans << std::endl;
  std::cout << "Best Race Plan Average Speed: "
            << mps2kph(population[0].get_average_speed()) << std::endl;
  std::cout << "Best Loop Plans: " << population[0].get_num_loops()
            << std::endl;
  if (Config::get_instance().get_print_population()) {
    int i = 0;
    for (const auto& plan : population) {
      std::cout << "Average Speed of " << i << ": "
                << mps2kph(plan.get_average_speed()) << std::endl;
      i++;
    }
  }
  std::cout << std::endl;
...
void V2Optimizer::simulate_population() {
  ...
  if (population_size > 1) {
    for (int i = 0; i < population_size; i++) {
      threads[i] = std::thread(thread_run_sim, simulator, route, result_luts[i],
                               &population[i], &thread_manager);
    }
    for (int i = 0; i < population_size; i++) {
      threads[i].join();
    }
...
V2Optimizer::V2Optimizer(std::shared_ptr<Simulator> simulator,
                         std::shared_ptr<FSGPRoute> route)
    : Optimizer(simulator, route),
      num_threads(std::max(
          1u, static_cast<unsigned int>(std::thread::hardware_concurrency() *
                                        Config::get_instance().get_threads()))),
