...
void ASCSimulator::run_sim_impl(const ASCRoute& route, ASCRacePlan* race_plan,
  ...
  const util::type::ForecastCoord sim_start_forecast_coord(params.sim_start_coord);
  ...
  // const std::unordered_set<size_t> control_stops =
  // route->get_control_stops();
  ...
  // Time race_start_time(curr_time);  // Save starting time to calculate total
  // elapsed time size_t segment_counter = 0; RacePlan::SegmentData
  // current_segment = segments[segment_counter]; curr_speed =
  // kph2mps(current_segment.start_speed);

  // /** Update the irradiance variable - passed in by reference */
  // auto update_irradiance = [&](const ForecastCoord& coord, const Time&
  // curr_time, Irradiance& irr) {
  //   double dni = 0.0;
  //   double dhi = 0.0;
  //   double ghi = 0.0;
  //   if (use_ghi) {
  //     ghi_lut.update_index_cache(&ghi_cache, coord,
  //     curr_time.get_utc_time_point()); irr.ghi =
  //     ghi_lut.get_value(ghi_cache);
  //   } else {
  //     params.dni_lut.update_index_cache(&dni_cache, coord,
  //     curr_time.get_utc_time_point()); irr.dni =
  //     params.dni_lut.get_value(dni_cache);

  //     params.dhi_lut.update_index_cache(&dhi_cache, coord,
  //     curr_time.get_utc_time_point()); irr.dhi =
  //     params.dhi_lut.get_value(dhi_cache);
  //   }
  // };
  ...
  //   */ ForecastCoord coord_one_forecast(current_coord.lat,
  //   current_coord.lon);

  //   params.wind_speed_lut.update_index_cache(&wind_speed_cache,
  //   coord_one_forecast, curr_time.get_utc_time_point()); double wind_speed =
  //   params.wind_speed_lut.get_value(wind_speed_cache);

  //   params.wind_dir_lut.update_index_cache(&wind_dir_cache,
  //   coord_one_forecast, curr_time.get_utc_time_point()); double wind_dir =
  //   params.wind_dir_lut.get_value(wind_dir_cache);
  ...
  //     // If the control stop spills into the next day, we need to calculate
  //     the stop in two stages
  //     // 1. curr_time -> EoD
  ...
  //     while (curr_time < current_day_end && time_charging <
  //     checkpoint_hold_time) {
  //       get_az_el(curr_time.get_utc_time_point(), current_coord.lat,
  //       current_coord.lon,
  //                 current_coord.alt, &sun.Az, &sun.El);
  //       double step_size = (curr_time +
  //       static_cast<double>(CHARGING_STEP_SIZE)) >= current_day_end ?
  //                           current_day_end - curr_time :
  //                           static_cast<double>(CHARGING_STEP_SIZE);
  //       delta_energy += car->compute_static_energy(current_coord, &curr_time,
  //       step_size, irr, "wsc"); curr_time.update_time_seconds(step_size);
  ...
  //     // overflowed_control_stop_time = control_stop_charge_time -
  //     time_charging;
  //   }
  ...
  //                 current_coord.alt, &sun.Az, &sun.El);
  //       double step_size = (curr_time +
  //       static_cast<double>(CHARGING_STEP_SIZE)) >= next_day_start ?
  //                           next_day_start - curr_time :
  //                           static_cast<double>(CHARGING_STEP_SIZE);
  //       if (sun.El > 0) {
  //         /* Charging at dawn/dusk */
  //         delta_energy += car->compute_static_energy(current_coord,
  //         &curr_time, step_size, irr, "wsc");
  ...
  //     double step_size = (time_charging +
  //     static_cast<double>(CHARGING_STEP_SIZE)) >
  ...
  //   CarUpdate update = car->compute_travel_update(current_coord, next_coord,
  //   curr_speed,
  ...
  //   /* Update the logs */
  //   results_lut.update_logs(update, irr, battery_energy, delta_energy,
  //   accumulated_distance,
  //                            next_coord, curr_speed, curr_time,
  //                            acceleration);

  //   /* Invalid simulation if battery goes below 0 or if the end of the race
  //   has been reached */ if (battery_energy < 0.0 || curr_time >
  //   race_end_time) {
  ...
  //   curr_speed = calc_final_speed(curr_speed, acceleration,
  //   update.delta_time);
  // }
  // race_plan->set_viability(true);
  // race_plan->set_time_taken(curr_time.get_local_time_point() -
  // race_start_time.get_local_time_point());
}
