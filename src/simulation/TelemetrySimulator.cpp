...
void TelemetrySimulator::run_sim(const TelemRoute& route,
  ...
  std::pair<size_t, size_t> dni_cache = params.dni_lut.initialize_caches(sim_start_forecast_coord, curr_time);
  std::pair<size_t, size_t> dhi_cache = params.dhi_lut.initialize_caches(sim_start_forecast_coord, curr_time);
  std::pair<size_t, size_t> wind_speed_cache = params.wind_speed_lut.initialize_caches(sim_start_forecast_coord, curr_time);
  std::pair<size_t, size_t> wind_dir_cache = params.wind_dir_lut.initialize_caches(sim_start_forecast_coord, curr_time);
  ...
  for (size_t i = 0; i < num_points - 1; i++) {
    ...
    wind.bearing = params.wind_dir_lut.get_value_and_update_cache(coord_one_forecast, curr_time, wind_dir_cache.first, wind_dir_cache.second);
    wind.speed = params.wind_speed_lut.get_value_and_update_cache(coord_one_forecast, curr_time, wind_speed_cache.first, wind_speed_cache.second);
    irr.dni = params.dni_lut.get_value_and_update_cache(coord_one_forecast, curr_time, dni_cache.first, dni_cache.second);
    irr.dhi = params.dhi_lut.get_value_and_update_cache(coord_one_forecast, curr_time, dhi_cache.first, dhi_cache.second);
...
TelemetrySimulator::TelemetrySimulator(TelemetrySimulatorParams params, Car model) :
