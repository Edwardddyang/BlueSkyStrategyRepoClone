...
std::string WSCRacePlan::get_plan_string_impl() const {
  RUNTIME_EXCEPTION(segments.size() > 0, "Plan must have >= 1 segments");
  std::stringstream output;

  auto print_header = [&]() {
    output << "+-------+-----------+---------------+-----------------------+---------------+\n";
    output << ";  Idx  ; Segments  ; Speeds (m/s)  ; Acceleration (m/s^2)  ; Distances (m) ;\n";
    output << "+-------+-----------+---------------+-----------------------+---------------+\n";
  };
  ...
  for (size_t seg_idx = 0; seg_idx < num_segments; seg_idx++) {
    ...
    // Segment speeds
    append_char_n_times(' ', 1, output);
    output << "[" << std::setw(5) << truncate_number(segments[seg_idx].start_speed, 5) << ","
                  << std::setw(5) << truncate_number(segments[seg_idx].end_speed, 5) << "] |";

    // Segment acceleration
    append_char_n_times(' ', 18, output);
    output << std::setw(5) << truncate_number(segments[seg_idx].acceleration_value, 5);
    output << "|";
...
bool WSCRacePlan::validate_members_impl(const WSCRoute& route) const {
  ...
  RUNTIME_EXCEPTION(segments.size() > 0, "No segments in WSCRacePlan");
  ...
  for (size_t seg_idx = 0; seg_idx < num_segments; seg_idx++) {
    ...
    const double segment_distance = calculate_segment_distance(route_points, start_idx,
                                                                end_idx);
    ...
    RUNTIME_EXCEPTION(start_speed >= 0.0 && end_speed >= 0.0,
                      ...
                      "Both must be >= 0.0", seg_idx, start_speed, end_speed);
    RUNTIME_EXCEPTION(start_idx <= end_idx, "Segment {} has start_idx = {}, end_idx = {}",
                      seg_idx, start_idx, end_idx);
    if (seg_idx < num_segments - 1) {
      RUNTIME_EXCEPTION(end_idx == segments[seg_idx + 1].start_idx,
                        "Segments must be continuous i.e. segment end = next segment start."
                        " Segment {} is inconsistent with next segment. Ending index = {}, "
                        " Next Starting Index = {}", seg_idx, end_idx, segments[seg_idx + 1].start_idx);
      RUNTIME_EXCEPTION(end_speed == segments[seg_idx + 1].start_speed,
                        "Segments must be continuous i.e. segment end speed = next segment start speed."
                        " Segment {} is inconsistent with next segment. Ending speed = {}, "
                        " Next Segment Starting Speed = {}", seg_idx, end_speed,
                        segments[seg_idx + 1].start_speed);
    }

    if (acceleration == 0.0) {
      RUNTIME_EXCEPTION(start_speed == end_speed,
                        "Speed profile for non-acceleration segment {} must have equal and positive "
                        "start and end speeds. Start speed = {}, End Speed = {}", seg_idx,
      ...
      RUNTIME_EXCEPTION(start_speed != end_speed, "Acceleration segment {} must have different start and ending speeds.",
                        seg_idx);
      const double calculated_acceleration_distance = util::calc_distance_a(start_speed, end_speed,
                                                                      acceleration);
      RUNTIME_EXCEPTION(std::abs(calculated_acceleration_distance - segment_distance) < tolerance,
                        "Acceleration segment {} has inconsistent acceleration values. Calculated acceleration distance is {}, "
...
FSGPRacePlan::FSGPRacePlan(PlanData segments, int num_repetitions) :
  ...
  RUNTIME_EXCEPTION(num_repetitions >= 1, "Number of repetitions per loop block be at least 1");
...
std::string FSGPRacePlan::get_loop_string(const LoopData& loop_segments) {
  ...
  for (size_t seg_idx = 0; seg_idx < num_segments; seg_idx++) {
    ...
    // Has Corner
    std::string corner_str = util::vector_to_string<size_t>(loop_segments[seg_idx].corners);
...
bool FSGPRacePlan::validate_members_impl(const FSGPRoute& route) const {
  ...
  for (size_t loop_idx=0; loop_idx < num_loops; loop_idx++) {
    ...
    for (size_t i=0; i < num_segments; i++) {
      ...
      const double acceleration_distance = loop_segments[i].acceleration_distance;
      ...
      RUNTIME_EXCEPTION(start_speed >= 0.0 && end_speed >= 0.0, "Segment speeds in segment {} are not positive", i);
      ...
      if (i < num_segments-1) {
        ...
        RUNTIME_EXCEPTION(is_crossover_segment, "Last segment of loop {} is not a crossover segment", loop_idx);
      }

      if (acceleration == 0.0) {
        RUNTIME_EXCEPTION(!is_dual_segment, "Segment {} with 0.0 acceleration cannot be a dual segment", i);
        RUNTIME_EXCEPTION(start_speed == end_speed,
        ...
        const double calculated_acceleration_distance = util::calc_distance_a(start_speed, end_speed,
                                                                        acceleration);
        if (is_dual_segment) {
          RUNTIME_EXCEPTION(std::abs(acceleration_distance + constant_distance - segment_distance) < tolerance,
                            "Acceleration distance {} and constant speed distance {} does not add "
                            "to the dual segment distance {}", acceleration_distance, constant_distance,
                            segment_distance);
          RUNTIME_EXCEPTION(calculated_acceleration_distance == acceleration_distance,
                            "Calculated acceleraiton distance {} is inconsistent in dual segment",
                            calculated_acceleration_distance);
        } else {
          RUNTIME_EXCEPTION(std::abs(calculated_acceleration_distance - segment_distance) < tolerance,
                            "Acceleration distance {} is invalid. Must be equal to the segment distance {} "
...
ASCRacePlan::ASCRacePlan(std::vector<LegSegments> base_segments,
  ...
  this->empty = (base_segments.size() == 0);
}
...
void ASCRacePlan::export_segments_json_impl(json& json_obj) const {
  ;
}

std::string ASCRacePlan::get_plan_string_impl() const {
  return "";
}

bool ASCRacePlan::validate_members_impl(const ASCRoute& route) const {
  return true;
}
