...
EnergyUpdate Car::compute_array_gain(double delta_time, util::type::Irradiance irr,
...
Geometry Car::calculate_geometry(util::type::Coord coord_one, util::type::Coord coord_two,
                                 const util::type::Time& time, double delta_distance) const {
  const double bearing = util::geo::get_bearing(coord_one, coord_two);
  const util::type::SolarAngle az_el = util::geo::get_az_el_from_bearing(bearing, coord_one, time);
  const double delta_altitude = coord_two.alt - coord_one.alt;
  const double distance = delta_distance == -1.0 ? util::geo::get_distance(coord_one, coord_two) : delta_distance;
  const double sin_angle = delta_altitude / distance;
  const double base_squared = distance * distance - delta_altitude * delta_altitude;
  if(base_squared < 0.0) {
    throw util::error::InvalidCalculation("Negative square root in base calculation");
  }
...
MotorEnergyLoss Car::calculate_motor_loss(double init_speed, double acceleration, double delta_time,
  ...
  if (acceleration < 0.0) {
    ...
    for (size_t i=0; i < num_data_points; i++) {
      const double speed = util::calc_final_speed(init_speed, acceleration, time);
      time = time + timestep;
    ...
    const double aero_energy_loss = util::constants::joules2kwh(integrator(timestep_data, aero_power_data));
    const double rolling_energy_loss = util::constants::joules2kwh(integrator(timestep_data, rolling_power_data));
    const double acceleration_energy_loss = util::constants::joules2kwh(integrator(timestep_data, acceleration_power_data));
    ...
    return MotorEnergyLoss(aero_energy_loss, rolling_energy_loss, gravitational_energy_loss,
                           acceleration_energy_loss, motor_energy);
  }
}

CarUpdate Car::compute_constant_travel_update(util::type::Coord coord_one,
  ...
  EnergyUpdate aero_loss = compute_aero_loss(speed, g.bearing, wind, delta_time);
  EnergyUpdate rolling_loss = compute_rolling_loss(speed, delta_time, g.cos_theta);
  EnergyUpdate gravity_loss = compute_gravitational_loss(speed, delta_time, g.sin_theta);
  ...
  // If resistive forces are propelling the car forwards, assume that energy/power is drawn
  if (motor_energy < 0.0) {
    motor_power = 0.0;
    motor_energy = 0.0;
  } else {
  ...
  return CarUpdate(
    aero_loss, rolling_loss, gravity_loss, array_gain, EnergyUpdate(), g.az_el,
    motor_power, motor_energy, g.bearing, electric_loss, delta_battery,
    g.distance, delta_time
  );
}

CarUpdate Car::compute_acceleration_travel_update(util::type::Coord coord_one,
                                                  util::type::Coord coord_two,
                                                  double init_speed,
                                                  double acceleration,
                                                  const util::type::Time& time,
  ...
  const Geometry g = calculate_geometry(coord_one, coord_two, time, delta_distance);
  const double delta_time = util::calc_time(init_speed, acceleration, delta_distance);
  ...
  const EnergyUpdate average_aero_loss(util::constants::kwh2joules(m.aero_loss) / delta_time, m.aero_loss);
  const EnergyUpdate average_rolling_loss(util::constants::kwh2joules(m.rolling_loss) / delta_time, m.rolling_loss);
  const EnergyUpdate average_acceleration_loss(util::constants::kwh2joules(m.acceleration_loss / delta_time),
                                               m.acceleration_loss);
  const EnergyUpdate average_gravity_loss(util::constants::kwh2joules(m.gravity_loss / delta_time),
                                          m.gravity_loss);

  return CarUpdate(average_aero_loss, average_rolling_loss, average_gravity_loss,
                  array_gain, average_acceleration_loss,
                  g.az_el, average_motor_power, m.motor_energy,
                  g.bearing, electric_loss, delta_battery, g.distance,
                  delta_time);
}

CarUpdate Car::compute_dual_travel_update(util::type::Coord coord_one,
                                          util::type::Coord coord_two,
                                          double init_speed,
                                          double acceleration,
                                          const util::type::Time& time,
  ...
  const double delta_time_acceleration = util::calc_time(init_speed, acceleration, acceleration_distance);
  const double final_acceleration_speed = util::calc_final_speed(init_speed, acceleration, delta_time_acceleration);
  const double delta_time_constant = util::calc_time_v(final_acceleration_speed, constant_distance);
  ...
  double acceleration_energy_loss = m.acceleration_loss;

  // Calculate constant speed component
  const EnergyUpdate aero_loss = compute_aero_loss(final_acceleration_speed, g.bearing, wind, delta_time_constant);
  const EnergyUpdate rolling_loss = compute_rolling_loss(final_acceleration_speed, delta_time_constant, g.cos_theta);
  const EnergyUpdate gravity_loss = compute_gravitational_loss(final_acceleration_speed, delta_time_constant, g.sin_theta);

  double motor_power = aero_loss.power + rolling_loss.power + gravity_loss.power;
  double motor_energy_out_constant = aero_loss.energy + rolling_loss.energy + gravity_loss.energy;
  ...
  if (motor_energy_out_constant < 0.0) {
    motor_energy_out_constant = 0.0;
  }
  ...
  const EnergyUpdate average_aero_loss(util::constants::kwh2joules(aero_energy_loss) / delta_time, aero_energy_loss);
  const EnergyUpdate average_rolling_loss(util::constants::kwh2joules(rolling_energy_loss) / delta_time, rolling_energy_loss);
  const EnergyUpdate average_acceleration_loss(util::constants::kwh2joules(acceleration_energy_loss / delta_time_acceleration),
                                                acceleration_energy_loss);
  const EnergyUpdate average_gravity_loss(util::constants::kwh2joules(gravity_energy_loss / delta_time),
                                          gravity_energy_loss);
  return CarUpdate(
    average_aero_loss, average_rolling_loss, average_gravity_loss,
    array_gain, average_acceleration_loss, g.az_el, average_motor_power,
    total_motor_energy, g.bearing, electric_loss, delta_battery,
    g.distance, delta_time
  );
}

double Car::compute_static_energy(util::type::Coord coord,
                                     util::type::Irradiance irr,
