...
class ForecastLut : public BaseLut<double>{
 ...
 public:
  ...
  /* Directly index the csv to return a value */
  double get_value(size_t row_idx, size_t col_idx);

  /** Index the csv using a {row idx, col idx} pair */
  double get_value(std::pair<size_t, size_t> index_cache);

  /** @brief Update index caches using new keys
   * @param index_caches Reference variable with old caches to update
   * @param coord Coordinate key
   * @param time Time key
   */
   ...
   */
  std::pair<size_t, size_t> initialize_caches(ForecastCoord coord, time_t time);
  std::pair<size_t, size_t> initialize_caches(Coord coord, time_t time);
};
