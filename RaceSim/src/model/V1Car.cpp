...
double V1Car::compute_net_battery_change(double array, double aero, double rolling,
  ...
  // Use regen or not
  double beta = aero + rolling + gravity > 0 ? 1 / motor_efficiency : regen_efficiency;
  // Charge or discharge
  double alpha = array > beta * motor + electric ? battery_efficiency : 1 / battery_efficiency;
...
double V1Car::compute_static_energy(Coord coord, Time* time, double charge_time, Irradiance irr, std::string sim_type) {
  ...
  time->print_local_readable_time();
...
V1Car::V1Car() : Car() { max_power = Config::get_instance()->get_max_motor_power(); }
