...
void V2Optimizer::mix(RacePlan* plan, RacePlanCreator::Gen* rng) {
  ...
  std::vector<std::string> mutations_vec(MUTATION_STRATEGIES.begin(), MUTATION_STRATEGIES.end());

  // Get raw plan
  RacePlan::PlanData new_raw_plan = plan->get_orig_segments();
  RacePlanCreator::PlanAttributes att;

  const size_t total_num_blocks = static_cast<int>(std::ceil(plan->get_num_loops() /
  ...
  for (size_t i=0; i < num_loops; i++) {
    ...
    const std::string mutation_strategy = mutations_vec[idx] != "Mix" ? mutations_vec[idx] : "ConstantForDeceleration";
    mutation_logger("Selected " + mutation_strategy + " for block " + std::to_string(i));
    ...
    generator->create_loop_block(&new_raw_plan[i], this->num_loops_in_block,
      i == total_num_blocks - 1, i == 0, &att, &mutation_logger
    );
    mutation_logger("Finished creating loop block");
  }

  mutation_logger("Finished mixed mutation");
  *plan = RacePlan(att.all_segments, att.raw_segments, this->num_loops_in_block);
}

void V2Optimizer::constant_noise_loop(RacePlan::PlanData* new_raw_plan, size_t loop_idx,
                                      RacePlanCreator::Gen* rng) {
  RUNTIME_EXCEPTION(new_raw_plan != nullptr, "Plan is null");
  RUNTIME_EXCEPTION(new_raw_plan->size() > loop_idx, "Not enough loops in the plan");
  ...
  for (size_t seg_idx=1; seg_idx < num_segments; seg_idx++) {
    ...
    RacePlan::SegmentData prev_orig_seg = (*new_raw_plan)[loop_idx][seg_idx-1];
    (*new_raw_plan)[loop_idx][seg_idx-1].end_speed += speed_increase;
    const double new_prev_acceleration_value = calc_acceleration(
      (*new_raw_plan)[loop_idx][seg_idx-1].start_speed,
      (*new_raw_plan)[loop_idx][seg_idx-1].end_speed,
      (*new_raw_plan)[loop_idx][seg_idx-1].distance
    );
    (*new_raw_plan)[loop_idx][seg_idx-1].acceleration_value = new_prev_acceleration_value;
...
void V2Optimizer::constant_noise(RacePlan* plan, RacePlanCreator::Gen* rng) {
  ...
  RUNTIME_EXCEPTION(total_num_blocks == new_raw_plan.size(), "raw_segments vector shoud "
                    "have number of loops equal to the number of blocks {}", total_num_blocks);
  for (size_t block_idx=0; block_idx < total_num_blocks; block_idx++) {
    ...
    generator->create_loop_block(&new_raw_plan[block_idx], this->num_loops_in_block,
      block_idx == total_num_blocks - 1, block_idx == 0, &att, &mutation_logger
    );
    mutation_logger("Finished creating loop block");
  }
...
void V2Optimizer::acceleration_noise_loop(RacePlan::PlanData* new_raw_plan, size_t loop_idx, RacePlanCreator::Gen* rng) {
  RUNTIME_EXCEPTION(new_raw_plan != nullptr, "Plan is null");
  RUNTIME_EXCEPTION(new_raw_plan->size() > loop_idx, "Not enough loops in the plan");
  RUNTIME_EXCEPTION(rng != nullptr, "rng collection is null");
  ...
  for (size_t seg_idx=0; seg_idx < num_segments; seg_idx++) {
    ...
    RacePlan::SegmentData orig_seg = (*new_raw_plan)[loop_idx][seg_idx];
    (*new_raw_plan)[loop_idx][seg_idx].end_speed += speed_increase;
    (*new_raw_plan)[loop_idx][seg_idx].acceleration_value = calc_acceleration(
      (*new_raw_plan)[loop_idx][seg_idx].start_speed,
...
void V2Optimizer::acceleration_noise(RacePlan* plan, RacePlanCreator::Gen* rng) {
  RUNTIME_EXCEPTION(rng != nullptr, "rng struct for gaussian noise is null");
  RUNTIME_EXCEPTION(plan != nullptr, "Plan in acceleration noise mutation is null");
...
void V2Optimizer::constant_for_deceleration(RacePlan* plan, RacePlanCreator::Gen* rng) {
  ...
  RUNTIME_EXCEPTION(plan != nullptr, "Plan in ConstantForDeceleration mutation is null");
...
void V2Optimizer::constant_for_deceleration_loop(RacePlan::PlanData* new_raw_plan,
                                                 size_t loop_idx, RacePlanCreator::Gen* rng) {
  RUNTIME_EXCEPTION(new_raw_plan != nullptr, "Plan is null");
  RUNTIME_EXCEPTION(new_raw_plan->size() > loop_idx, "Not enough loops in the plan");
  RUNTIME_EXCEPTION(rng != nullptr, "rng collection is null");
...
bool V2Optimizer::legalize_loop(RacePlan::PlanData* plan,
  ...
  /** @brief Check that seg does not violate the cornering indices or speeds in corners */
  auto check_cornering_constraint = [&](const RacePlan::SegmentData& seg,
                                        const std::vector<size_t>& corners) -> bool {
    for (const auto& c : seg.corners) {
      const size_t corner_start = this->route->get_cornering_segment_bounds()[c].first;
      if (seg.start_idx > corner_start) {
        return false;
      }
      const size_t max_corner_speed = this->route->get_cornering_speed_bounds()[c];
  ...
  /** @brief Check that the segment does not violate acceleration/deceleration constraints */
  auto check_acceleration_constraint = [&](const RacePlan::SegmentData& seg) -> bool {
    if (seg.acceleration_value > 0.0) {
      const double drawn_power = seg.acceleration_value * seg.end_speed * this->car_mass;
      if (drawn_power > this->acceleration_power_budget || seg.acceleration_value > this->max_acceleration) {
        (*logger)("Acceleration failed on the legalization start segment");
        return false;
      }
    } else if (seg.acceleration_value < 0.0 && seg.acceleration_value < this->max_deceleration) {
  ...
  if (!check_acceleration_constraint(saved_plan[loop_idx][seg_idx])) {
    (*logger)("Acceleration constraints failed on the legalization start segment");
    return false;
  }
  if (!check_cornering_constraint(saved_plan[loop_idx][seg_idx], saved_plan[loop_idx][seg_idx].corners)) {
  ...
  */
  auto resolve_next_segment = [&](RacePlan::LoopData& loop, size_t i) -> bool {
    ...
    if (loop[i].end_idx != loop[i+1].start_idx) {
      ...
      // Speed discontinuity
      if (new_segment.corners.size() > 0 && loop[i+1].acceleration_value == 0.0) {
        // Corner segment
  ...
  // The following are necessary for resolving segments that connect the current segments to the
  // next loop
  ...
  // FInd the route index of the first corner whose maximum cornering speed is less
  // than the maximum route speed. This is the corner to which wrap-around segments are applied
