...
TEST_F(OptimizerTest, Test1) {
  ...
  std::filesystem::path results_file =
      strat_root_path / "data/luts/TestData/61.csv";
  ResultsLut golden_result(results_file);

  std::shared_ptr<ResultsLut> test_result =
      result_luts[61 - Config::get_instance().get_min_speed()];

  const double margin = 0.00001;
  // Test battery_energy
  std::vector<double> golden_battery_energy =
      golden_result.get_battery_energy();
  ...
  std::vector<double> golden_accumulated_distance =
      golden_result.get_accumulated_distance();
  std::vector<double> test_accumulated_distance =
      test_result->get_accumulated_distance();
  EXPECT_EQ(golden_accumulated_distance.size(),
            test_accumulated_distance.size());
  for (size_t i = 0; i < golden_accumulated_distance.size(); ++i) {
    EXPECT_NEAR(golden_accumulated_distance[i], test_accumulated_distance[i],
                margin);
  }
  ...
  // Test rolling_energy
  std::vector<double> golden_rolling_energy =
      golden_result.get_rolling_energy();
  ...
  std::vector<double> golden_gravitational_power =
      golden_result.get_gravitational_power();
  std::vector<double> test_gravitational_power =
      test_result->get_gravitational_power();
  ...
  for (size_t i = 0; i < golden_gravitational_power.size(); ++i) {
    EXPECT_NEAR(golden_gravitational_power[i], test_gravitational_power[i],
                margin);
  }

  // Test gravitational_energy
  std::vector<double> golden_gravitational_energy =
      golden_result.get_gravitational_energy();
  std::vector<double> test_gravitational_energy =
      test_result->get_gravitational_energy();
  EXPECT_EQ(golden_gravitational_energy.size(),
            test_gravitational_energy.size());
  for (size_t i = 0; i < golden_gravitational_energy.size(); ++i) {
    EXPECT_NEAR(golden_gravitational_energy[i], test_gravitational_energy[i],
                margin);
  }

  // Test electric_energy
  std::vector<double> golden_electric_energy =
      golden_result.get_electric_energy();
