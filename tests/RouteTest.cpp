...
TEST_F(RouteTest, UniformSegmentTestNewRoute) {
  std::filesystem::path base_route_path =
      RouteTest::strat_root_path /
      "data/luts/TestData/uniform_segment_testing.csv";
  Route new_route = Route(base_route_path);
  std::vector<std::pair<size_t, size_t>> segments =
      new_route.segment_route_uniform(new_route.get_route_length());
...
TEST_F(RouteTest, ForecastLutTest) {
  std::filesystem::path dni_path =
      RouteTest::strat_root_path / "data/luts/TestData/dni.csv";
  ...
  std::pair<size_t, size_t> caches = dni_lut.initialize_caches(
      starting_coord, starting_time.get_utc_time_point());
  ...
  // Times exactly in between the current and next row should use the current
  // row cache
  Coord next_coord(-12.655914, 131.077636);
  Time next_time("2023-10-22 09:15:00", -9.5);
  dni_lut.update_index_cache(&caches, next_coord,
                             next_time.get_utc_time_point());
